...
const examples = [
    ...
    { name: "9. Ecuación Circular 2D", expr: "x^2 + y^2 = a", mode: "2D" },
    { name: "10. Oscilador 1D", expr: "a * sin(t) * 5", mode: "1D" }
];
...
function prepareExpression(rawExpr) {
    // Manejo de ecuaciones implícitas y explícitas con '='
    let parts = rawExpr.toLowerCase().split('=');
    let expr;
    if (parts.length === 2) {
        ...
        expr = rawExpr.toLowerCase();
    }

    // Sustituciones matemáticas
    expr = expr.replace(/\^/g, '**');
...
function updateGraphics(t = 0) {
    for(let i=0; i<MAX_GRAPHS; i++) {
        ...
        try {
            ...
            // Test de sintaxis
            f(0,0,0,AppState.a,AppState.b,t);
        } catch (e) {
            curves2D[i].line.visible = false;
            meshes3D[i].mesh.visible = false;
            pointers1D[i].visible = false;
...
// ENTER: Compila y fija la gráfica actual
function calculate() {
    ...
    try {
        ...
        updateGraphics(AppState.time); // Asegura que se dibuje
        els.calc.classList.add('pulse');
        setTimeout(() => els.calc.classList.remove('pulse'), 300);
    } catch (e) {
        // No hacer nada si hay error de sintaxis, el display lo mostrará
    }
}

// SGT (Siguiente Gráfica): Fija la actual y pasa a la siguiente
function nextGraph() {
